...
def create_train_sample(sample, tokenizer, max_length: int):
    encoding = tokenizer(sample.source, max_length=max_length, truncation=True).input_ids
    attention_mask = torch.ones(len(encoding), dtype=torch.bool)
    with tokenizer.as_target_tokenizer():
        label = tokenizer(sample.target, max_length=max_length, truncation=True).input_ids
